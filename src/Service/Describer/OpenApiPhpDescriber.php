...
use Elenyum\OpenAPI\Service\Util\SetsContextTrait;
...
/**
 * @description generate documentation on route collection
 *
...
final class OpenApiPhpDescriber
{
    ...
    public function __construct(
        ...
        private FilteredRouteCollectionBuilder $filteredRouteCollectionBuilder,
        private bool $overwrite = false
    ) {
        $this->routeCollection =  $filteredRouteCollectionBuilder->filter($this->routeCollection);
    }
    ...
    public function describe(OA\OpenApi $api)
    {
        ...
        foreach ($this->getMethodsToParse() as $method => [$path, $httpMethods, $routeName]) {
            ...
            if (!array_key_exists($declaringClass->getName(), $classAnnotations)) {
                ...
                $classAnnotations = array_merge(
                    $classAnnotations,
                    $this->getAttributesAsAnnotation($declaringClass, $context)
                );
            ...
            foreach (array_merge($annotations, $classAnnotations[$declaringClass->getName()]) as $annotation) {
                ...
                if (
                    ...
                    throw new \LogicException(
                        sprintf(
                            'Using the annotation "%s" as a root annotation in "%s::%s()" is not allowed.',
